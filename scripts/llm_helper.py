...
class ChatGLMHelper:
    ...
    def translate_title(self, title: str, abstract: str = "") -> str:
        ...
        # 简洁的提示词，直接要求返回中文标题
        prompt = f"""请将以下AI/算法论文标题翻译成中文。

要求：
1. 保持专业术语、模型名称、算法名称、缩写为英文原样（如CLIP、ViT、NeRF、MoE、CoT等）
2. 只输出翻译后的中文标题，不要包含任何解释或额外内容
3. 确保输出是纯中文标题（除保留的英文术语外）

标题：{title}

输出格式：直接输出中文标题，不要加引号或其他标记"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if translation and any('\u4e00' <= char <= '\u9fff' for char in translation):
                    ...
                    print(f"警告：第{attempt + 1}次翻译未返回中文结果，重试中... 原始结果: {translation[:50]}")
    ...
    def clean_json_string(self, text: str) -> str:
                ...
                if '"reason"' in line and ':' in line:
                    ...
                    if len(parts) == 2 and '"' in parts[1]:
                        ...
                        if quote_count >= 2:
                            ...
                            reason_start = i
                            # 提取第一个引号之后的内容
                            first_quote = parts[1].find('"')
                            if first_quote != -1:
                                reason_content.append(parts[1][first_quote+1:])
                elif in_reason:
                    # 在reason字段中
                    if '"' in line:
                        # 找到结束引号
                        ...
                        if last_quote != -1:
                            reason_content.append(line[:last_quote])
                            in_reason = False
                            # 重建reason字段
                            reason_value = '\n'.join(reason_content)
                            # 转义特殊字符
                            escaped_reason = (reason_value
                                .replace('\\', '\\\\')
                                .replace('"', '\\"')
                                .replace('\n', '\\n')
                                .replace('\r', '\\r')
                                .replace('\t', '\\t'))
                            # 替换原来的多行reason
                            lines[reason_start] = lines[reason_start].split(':', 1)[0] + ': "' + escaped_reason + '"'
                            # 删除中间的行
                            del lines[reason_start+1:i+1]
                            break
                    else:
        ...
        # 如果还是失败，尝试最激进的修复：移除reason字段
        text = re.sub(r',?\s*"reason"\s*:\s*"[^"]*"', '', text)
        text = re.sub(r'\{\s*,', '{', text)  # 修复可能的逗号问题
    ...
    def get_category_by_keywords(self, title: str, abstract: str) -> List[Tuple[str, float]]:
        ...
        # 第一阶段：计算每个类别的初始得分
        for category, config in ai_categories_config.CATEGORY_KEYWORDS.items():
        ...
        if category_scores:
            ...
            # 使用"其他"类别的阈值作为基准
            other_config = ai_categories_config.CATEGORY_THRESHOLDS.get("其他", {})
            ...
            for category, score in category_scores.items():
                # 获取类别的阈值
                category_config = ai_categories_config.CATEGORY_THRESHOLDS.get(category, {})
    ...
    def get_category_by_semantic(self, title: str, abstract: str) -> List[Tuple[str, float]]:
        """使用语义分析进行分类"""
        categories_str = "\n".join(f"- {cat}" for cat in ai_categories_config.CATEGORY_DISPLAY_ORDER)
...
{{
    ...
    def combine_results(self, keyword_results: List[Tuple[str, float]], 
        ...
        significant_results = [
            ...
            if score >= max_score * 0.7  # Significance threshold
        ]
...
{ai_categories_config.CATEGORY_PROMPT}
...
请直接返回最合适的类别名称，不要有任何解释或额外文本。"""
            ...
            # 验证返回的类别是否在预定义类别中
            if category in ai_categories_config.CATEGORY_DISPLAY_ORDER:
...
请选择最能准确反映论文核心内容的类别，直接返回类别名称，不要有任何解释。"""
                    ...
                    if final_category in ai_categories_config.CATEGORY_DISPLAY_ORDER:
            ...
            for cat in ai_categories_config.CATEGORY_DISPLAY_ORDER:
...
请直接返回最合适的类别名称，不要有任何解释或额外文本。只返回类别名称。"""
    ...
    def determine_subcategory(self, title: str, abstract: str, main_category: str) -> str:
        ...
        try:
            ...
            if main_category == "其他" or main_category not in ai_categories_config.CATEGORY_THRESHOLDS:
                return "未指定"
                
            # 获取主类别的子类别配置
            subcategories = ai_categories_config.CATEGORY_THRESHOLDS[main_category].get("subcategories", {})
...
请直接返回最合适的子类别名称，不要有任何解释或额外文本。如果无法确定，请返回"未指定"。"""
            ...
            if best_match and best_score >= 3:  # 设置最低匹配阈值（提高从2到3）
...
请直接返回子类别名称，不要有任何解释。如果无法确定，请返回"未指定"。"""
    ...
    def classify_paper_with_llm(self, title: str, abstract: str) -> List[Tuple[str, float, Optional[Tuple[str, float]], Optional[Dict]]]:
        """使用LLM对论文进行分类，返回主类别和子类别
        ...
        Returns:
            List[Tuple[str, float, Optional[Tuple[str, float]], Optional[Dict]]]: 
            [(主类别, 置信度, (子类别, 置信度), 分类解释), ...]
        """
        try:
            categories_str = "\n".join(f"- {cat}" for cat in ai_categories_config.CATEGORY_DISPLAY_ORDER)
            
            prompt = f"""请作为一位专业的AI/算法研究专家，对以下论文进行精确分类。
...
2. 优先考虑论文的技术本质和创新点，而非应用场景
3. 如果论文跨多个领域，请选择最能体现其核心创新的类别
4. 必须同时确定主类别和子类别，不能留空

请严格按照以下JSON格式返回分类结果（不要添加任何其他文字或解释）：
{{
    "main_category": "主类别名称",
...
重要提示：
- main_confidence 和 subcategory_confidence 的取值范围是 0.0 到 1.0
- 必须返回主类别和子类别，都不能为空字符串
- 请确保返回的类别名称与上述可选类别完全一致（包括中文部分）
...
- 只返回JSON对象，不要包含markdown代码块标记或其他文字"""
            ...
            try:
                ...
                main_confidence = float(data.get("main_confidence", 0.0))
                subcategory = data.get("subcategory", "")
                subcategory_confidence = float(data.get("subcategory_confidence", 0.0))
                reason = data.get("reason", "")
                
                # 验证主类别是否在预定义类别中
                if main_category not in ai_categories_config.CATEGORY_DISPLAY_ORDER:
                    # print(f"LLM返回的类别 '{main_category}' 不在预定义类别中，尝试修正")
                    # 尝试找到最相似的类别
                    best_match = None
                    best_score = 0
                    ...
                    for cat in ai_categories_config.CATEGORY_DISPLAY_ORDER:
                        ...
                        # 完全匹配
                        if main_lower == cat_lower:
                            score = 1.0
                        # 包含关系
                        elif main_lower in cat_lower:
                            score = 0.9
                        elif cat_lower in main_lower:
                            score = 0.8
                        # 部分匹配（单词级别）
                        else:
                            main_words = set(main_lower.split())
                            cat_words = set(cat_lower.split())
                            if main_words and cat_words:
                                intersection = main_words & cat_words
                                union = main_words | cat_words
                    ...
                    # 如果找到足够相似的类别（相似度 > 0.6），使用它
                    if best_match and best_score > 0.6:
                        main_category = best_match
                        # print(f"修正为: '{main_category}' (相似度: {best_score:.2f})")
                    else:
                        # 如果没有找到足够相似的类别，不返回结果
                        # print(f"无法找到足够相似的类别 (最佳相似度: {best_score:.2f})")
                        return []
                
                # 如果子类别为空，自动分配该主类别下最相似的子类别
                if main_category and (not subcategory or subcategory == ""):
                    # print(f"子类别为空，自动分配主类别 '{main_category}' 下最相似的子类别")
                    category_config = ai_categories_config.CATEGORY_THRESHOLDS.get(main_category, {})
                    if isinstance(category_config, dict):
                        valid_subcategories = list(category_config.get("subcategories", {}).keys())
                        if valid_subcategories:
                            ...
                            for subcat in valid_subcategories:
                                ...
                                # 计算子类别名称与文本的重叠度
                                intersection = subcat_words & text_words
                                if intersection:
                                    score = len(intersection) / len(subcat_words)
                                
                                # 额外检查：如果子类别名称中包含关键词，提高分数
                                if score < 0.5:
                                    keywords = ["推理", "优化", "压缩", "量化", "加速", "生成", "检测", "安全", "对齐", "多模态", "视觉", "语言", "模型", "训练", "预训练", "机器人", "智能体", "agent", "reasoning", "optimization", "compression", "quantization", "acceleration", "generation", "detection", "safety", "alignment", "multimodal", "vision", "language", "model", "training", "pretraining", "robotics"]
                                    for keyword in keywords:
                                        if keyword in text and keyword in subcat_lower:
                                            score = max(score, 0.6)
                                            break
                            ...
                            # 如果找到匹配的子类别且分数足够高，使用它
                            if best_match and best_score >= 0.6:
                                subcategory = best_match
                                subcategory_confidence = 0.7  # 自动分配的子类别置信度设为0.7
                                # print(f"自动分配子类别: '{subcategory}' (相似度: {best_score:.2f})")
                            else:
                                # 如果没有找到足够匹配的子类别，返回空
                                # print(f"未找到足够匹配的子类别 (最佳相似度: {best_score:.2f})")
                                return []
                        else:
                            # print(f"主类别 '{main_category}' 没有定义子类别")
                            return []
                    else:
                        # print(f"主类别 '{main_category}' 配置无效")
                        return []
                ...
                if main_category and subcategory:
                    category_config = ai_categories_config.CATEGORY_THRESHOLDS.get(main_category, {})
                    if isinstance(category_config, dict):
                        valid_subcategories = category_config.get("subcategories", {}).keys()
                        if subcategory not in valid_subcategories:
                            # print(f"LLM返回的子类别 '{subcategory}' 不在主类别 '{main_category}' 的子类别中，尝试修正")
                            # 尝试找到最相似的子类别
                            best_match = None
                            best_score = 0
                            ...
                            for subcat in valid_subcategories:
                                ...
                                # 完全匹配
                                if sub_lower == subcat_lower:
                                    score = 1.0
                                # 包含关系
                                elif sub_lower in subcat_lower:
                                    score = 0.9
                                elif subcat_lower in sub_lower:
                                    score = 0.8
                                # 部分匹配（单词级别）
                                else:
                                    sub_words = set(sub_lower.split())
                                    subcat_words = set(subcat_lower.split())
                                    if sub_words and subcat_words:
                                        intersection = sub_words & subcat_words
                                        union = sub_words | subcat_words
                                        score = len(intersection) / len(union) if union else 0
                                
                                # 额外检查：如果子类别名称中包含关键词，提高分数
                                if score < 0.6:
                                    # 提取关键词
                                    keywords = ["推理", "优化", "压缩", "量化", "加速", "生成", "检测", "安全", "对齐", "多模态", "视觉", "语言", "模型", "训练", "预训练", "机器人", "智能体", "agent", "推理", "reasoning", "optimization", "compression", "quantization", "acceleration", "generation", "detection", "safety", "alignment", "multimodal", "vision", "language", "model", "training", "pretraining", "robotics", "agent"]
                                    for keyword in keywords:
                                        if keyword in sub_lower and keyword in subcat_lower:
                                            score = max(score, 0.7)
                                            break
                            ...
                            if best_match and best_score >= 0.6:
                                subcategory = best_match
                                # print(f"修正为: '{subcategory}' (相似度: {best_score:.2f})")
                            else:
                                # 如果没有找到足够匹配的子类别，返回空
                                # print(f"未找到足够匹配的子类别 (最佳相似度: {best_score:.2f})")
                                return []
                ...
                if not main_category or not subcategory:
                    # print(f"主类别或子类别为空，返回空结果")
                    return []
                
                # 检查置信度是否达到阈值要求
                category_config = ai_categories_config.CATEGORY_THRESHOLDS.get(main_category, {})
                if isinstance(category_config, dict):
                    main_threshold = category_config.get("threshold", 0.7)
                    subcategories_config = category_config.get("subcategories", {})
                    if isinstance(subcategories_config, dict):
                        subcategory_threshold = subcategories_config.get(subcategory, 0.7)
                    else:
                        subcategory_threshold = 0.7
                else:
                    main_threshold = 0.7
                    subcategory_threshold = 0.7
                
                # 如果主类别或子类别的置信度低于阈值，返回空结果
                if main_confidence < main_threshold or subcategory_confidence < subcategory_threshold:
                    return []
                ...
                if main_category:
                    ...
                    if subcategory and subcategory != "":
                        subcategory_info = (subcategory, subcategory_confidence)
                    
                    explanation = {"reason": reason} if reason else None
                    result_list.append((main_category, main_confidence, subcategory_info, explanation))
                
                return result_list
                
            except json.JSONDecodeError as e:
                # print(f"JSON解析失败: {json_str}, 错误: {str(e)}")
                return []
                
        except Exception as e:
            # print(f"LLM分类出错: {str(e)}")
            traceback.print_exc()
            return []
